...
PLACE_ID="$(printf "%s" "$BODY" | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')"
if [[ -z "$PLACE_ID" ]]; then
...
REVIEW_ID="$(printf "%s" "$BODY" | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')"
if [[ -z "$REVIEW_ID" ]]; then
